...
Endpoints:
  GET  /                          -> health check
  POST /create-video              -> starts a render job, returns {job_id, status} immediately
  GET  /video-status/{job_id}     -> {status: "processing" | "completed" | "failed", ...}
  GET  /video-download/{job_id}   -> returns final MP4 (binary) once status is "completed"

Rendering runs in a background thread so the HTTP request returns in
milliseconds. This avoids platform-level reverse-proxy timeouts (e.g.
Railway's ~5 minute gateway timeout) that would otherwise kill a long-lived
synchronous request before a multi-scene render finishes.
"""
...
JOBS: Dict[str, dict] = {}
JOB_TTL_SECONDS = 60 * 60  # drop finished job bookkeeping after 1 hour
...
def _cleanup_old_jobs():
    ...
    for jid in list(JOBS.keys()):
        job = JOBS[jid]
        if job.get("created_at", 0) < cutoff and job.get("status") in ("completed", "failed"):
...
def _render_job(job_id: str, req: VideoRequest):
    """Runs the full render pipeline for one job. Executes in a background
    thread; owns its own asyncio event loop for the async TTS calls."""
    ...
    try:
        ...
        for i, scene in enumerate(req.scenes):
            ...
            loop.run_until_complete(generate_audio(scene.voiceover, req.voice, voice_path))
            ...
            # Progress visibility while polling
            JOBS[job_id]["progress"] = f"scene {i + 1}/{len(req.scenes)} done"
        ...
        JOBS[job_id]["error"] = f"Video generation failed: {str(e)}"
    finally:
        loop.close()
        _cleanup_old_jobs()


@app.post("/create-video", status_code=202)
async def create_video(req: VideoRequest):
    if not req.scenes:
        raise HTTPException(status_code=400, detail="'scenes' cannot be empty")

    job_id = str(uuid.uuid4())
    JOBS[job_id] = {"status": "processing", "created_at": time.time(), "progress": "starting"}

    thread = threading.Thread(target=_render_job, args=(job_id, req), daemon=True)
    thread.start()

    # Returns almost instantly - the actual render keeps running in the
    # background thread, so this request never sits open long enough for
    # a reverse-proxy (e.g. Railway) to time it out.
...
def video_status(job_id: str):
    ...
    if job["status"] == "completed":
        resp["download_url"] = f"/video-download/{job_id}"
